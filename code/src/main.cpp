...
int main()
{
	...
	if (!writer.isOpened())
	{
		if (!writer.open(video_name+".avi", CV_FOURCC('M', 'J', 'P', 'G'), 25.0, cv::Size(800,800), true))
		{
	...
	for (int i =0; i < data_log_size; i++)
	{
       ...
       if(motion_theta!=motion_theta)
		...
		if(fabs(control.delta_x) < 0.3 && fabs(control.delta_y) < 0.3 && fabs(motion_theta) < 0.03)
		{
